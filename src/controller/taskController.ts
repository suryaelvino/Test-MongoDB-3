...
export const searchTasksByKeyword = async (req: Request, res: Response) => {
    const { q } = req.query;
    // Validasi parameter query q
    if (!q || typeof q !== 'string' || q.trim() === '') {
        return res.status(code.BAD_REQUEST).json({ message: 'Parameter query q is required and must be a non-empty string' });
    }
    try {
